...
def run(ctx):
    ...
    # Load Labware
    tiprack_300ul = [ctx.load_labware('opentrons_96_tiprack_300ul', slot) for slot in range(1,3)]
    tiprack_20ul = [ctx.load_labware('opentrons_96_tiprack_20ul', slot) for slot in range(3,5)]
    ...
    # Load Pipettes
    p300 = ctx.load_instrument('p300_single_gen2', p300_mount, tip_racks=tiprack_300ul)
    ...
    for row in lc_plates:
        s_plate, s_well, d2_vol, d2_plate, d2_well = row[0], row[1], row[10], row[11], row[12]
    ...
    for row in lc_plates:
        d1_plate, d1_well, d2_vol, d2_plate, d2_well = row[6], row[7], row[10], row[11], row[12]
        ...
        pip = p300 if d2_vol > 20 else p20
        pip.transfer(d2_vol, d1_plate.wells_by_name()[d1_well], d2_plate.wells_by_name()[d2_well])
