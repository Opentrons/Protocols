...
def run(ctx):
    ...
    # Load Labware
    tipracks = [ctx.load_labware('opentrons_96_filtertiprack_20ul', slot) for slot in range(1,3)]
    mastermix = ctx.load_labware('nest_12_reservoir_15ml', 7)
    sample_plate = ctx.load_labware('kingfisher_96_deepwell_plate_2ml', 4)
    pcr_plate = ctx.load_labware('thermofishermicroampfast96well0.1_96_wellplate_100ul', 5)
