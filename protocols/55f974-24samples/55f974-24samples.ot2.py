...
metadata = {
    'protocolName': 'MagBead Based Peptide Purification with 24 Samples',
...
mag_plate = deep_plate.rows('A')[:3]

t_plate = labware.load(deep_name, '5', 'Transfer Plate')
deep_trans = t_plate.rows('A')[:3]
...
def run_custom_protocol(
    ...
    with sample plate in slot 5. Dump the liquid waste in slot 7 and replace \
    the reservoir. When ready to resume, click RESUME.')
