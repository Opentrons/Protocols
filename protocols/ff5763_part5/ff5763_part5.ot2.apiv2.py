...
def run(ctx):
    ...
    final_plate = ctx.load_labware('customabnest_96_wellplate_200ul',
                                   '2')
    ...
    source_midi_plate = midi_plate_1.rows()[0][:num_cols]
    dest_midi_plate = midi_plate_1.rows()[0][6:6+num_cols]
...
# transfer 15ul IPB to each empty well second half of MIDI plate
    ...
    for dest in dest_midi_plate:
        ...
        m20.flow_rate.blow_out *= 4
    m20.drop_tip()
# Transfer 125ul supernatant from left side to right side with a 10x mix

    mag_module.engage(height=MIDI_plate_mag_height)
    ctx.max_speeds['Z'] = 50
    ctx.max_speeds['A'] = 50
    num_times = 1
    for source, dest in zip(source_midi_plate, dest_midi_plate):
...
# transfer 30 ul from MIDI plate 2 to new 96 well pcr plate
    ...
    ctx.max_speeds['A'] = 50
    num_times = 1
    for source, dest in zip(dest_midi_plate, final_plate_dest):
        side = 1 if num_times % 2 == 0 else -1
        m300.flow_rate.aspirate /= 5
        m300.pick_up_tip()
        ctx.max_speeds['Z'] /= z_mod_value
