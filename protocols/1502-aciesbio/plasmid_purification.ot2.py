...
def run_custom_protocol(
    ...
    num_top_ind = int((num_samples-1)/8)+1

    plate_loc = [well for well in plate.rows['A'][:num_top_ind]]
    plate_top_loc = [well.top() for well in plate_loc]

    mag_loc = [well for well in plate.rows['A'][:num_top_ind]]
    ...
    flat_loc = [well for well in flat_plate.rows['A'][0:num_top_ind]]
