...
metadata = {
    'protocolName': 'QIAseq Targeted DNA Panel Part 1',
...
def run_custom_protocol(
    ...
    m300.distribute(50, water, water_dest, disposal_vol=0)
    update_m300_tip_count(1)
    if wells_num:
        water_dest = [well.top()
                      for well in mag_plate.cols(cols_num)[:wells_num]]
        p50.transfer(50, water, water_dest)

    # add QIAseq Beads to samples
    bead_dest = [col[0].top() for col in mag_plate.cols[:cols_num]]
    m300.distribute(50, beads, bead_dest, disposal_vol=0)
    update_m300_tip_count(1)
    if wells_num:
        bead_dest = [well.top()
                     for well in mag_plate.cols(cols_num)[:wells_num]]
        p50.transfer(50, beads, bead_dest)
    ...
    # elute DNA
    water_dest = [col[0] for col in mag_plate.cols[:cols_num]]
    for col in water_dest:
        m300.pick_up_tip()
        m300.transfer(52, water, col, new_tip='never')
        m300.mix(3, 30, col)
        m300.blow_out(col.top())
        m300.drop_tip()
        update_m300_tip_count(1)
    if wells_num:
        water_dest = [well for well in mag_plate.cols(cols_num)[:wells_num]]
        for well in water_dest:
            p50.pick_up_tip()
            p50.transfer(52, water, well, new_tip='never')
            p50.mix(3, 30, well)
...
slot 1 on Magnetic Module and supply a new clean plate in slot 1.')
    ...
    # remove supernatant
    for col in multi_aspirate_loc:
        m300.transfer(100, col.bottom(1), m300.trash_container.top())
        update_m300_tip_count(1)

    # wash beads with ethanol
    for ethanol in [ethanol_3, ethanol_4]:
        m300.distribute(200, ethanol, multi_dispense_loc)
        update_m300_tip_count(1)
        for col in multi_dispense_loc:
            m300.transfer(200, col, m300.trash_container.top())
    ...
    for well in mag_plate.wells('A1', length=number_of_samples):
        p50.pick_up_tip()
        p50.transfer(12, water, well, new_tip='never')
        p50.mix(3, 10, well)
        p50.blow_out(well.top())
        p50.drop_tip()
    ...
    for source, dest in zip(sources, dests):
        p50.pick_up_tip()
        p50.transfer(9.4, source, dest, new_tip='never')
