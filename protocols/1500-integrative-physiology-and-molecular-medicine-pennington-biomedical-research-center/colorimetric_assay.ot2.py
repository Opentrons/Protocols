...
tips = [labware.load('tiprack-200ul', slot) for slot in ['7', '10']]
...
def run_custom_protocol(
    ...
    for sample, target in zip(tubes_total, well_order):
        ...
        p300.transfer(175, sample, wells)

    # sets number of occupied columns of samples in plate
    num_transfers = (int((sample_num-1)/8)+1)*3

    # transfers solution 1 from trough to all columns of samples
    ...
    # transfers solution 12 from trough to all columns of samples
    for col in range(num_transfers):
        m300.transfer(
                50,
                trough.wells('A12'),
