...
metadata = {
    'protocolName': 'leChRO-seq Day 3 Part 1: 2nd Bead Binding and Enrichment',
...
# reagent setup
streptavidin_beads = buffer_plate.cols('1')
high_salt_buffer = buffer_plate.cols('2')
binding_buffer = buffer_plate.cols('3')
low_salt_buffer = buffer_plate.cols('4')
...
# transfer Binding Buffer to samples
m300.pick_up_tip()
m300.transfer(40, binding_buffer, sample_col, new_tip='never')

# mix and transfer Streptavidin Beads
m300.mix(3, 50, streptavidin_beads)
m300.transfer(50, streptavidin_beads, sample_col, new_tip='never')
m300.mix(15, 80, sample_col)
for i in range(30):
    time.sleep(30)
    m300.mix(15, 80, sample_col)
m300.retract()
...
# remove supernatant
m300.transfer(120, sample_col, liquid_trash.top(), new_tip='never')
m300.drop_tip()
# turn off Magnetic Module
mag_module.disengage()

# wash samples with High Salt Buffer twice
for _ in range(2):
    m300.pick_up_tip()
