...
def run(ctx):
    ...
    tip_racks = [ctx.load_labware('opentrons_96_filtertiprack_20ul', x) for x in ['7','4']]
    p20m = ctx.load_instrument('p20_multi_gen2', 'right', tip_racks=tip_racks)

    [p20m.transfer(15, rehydration_buffer, col, mix_after=(2,10)) for col in pcr_plate.rows()[0]]
