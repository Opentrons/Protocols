...
metadata = {
    'protocolName': 'NEB Next Ultra II FS Library Prep: Size Selection of \
Adapter-ligated DNA',
...
between 1 and 96)')
    ...
    for m in mag_samples_multi:
        m50.transfer(28.5, te, m, new_tip='never')
        m50.blow_out()
    m50.drop_tip()
    ...
    m300.distribute(
        40, spri_beads, [m.top() for m in mag_samples_multi], new_tip='never')
    for m in mag_samples_multi:
    ...
    # mix and distribute beads to elution plate
    pick_up(m50)
    mix(5, 45, spri_beads, m50)
    m50.distribute(
        20,
        spri_beads,
        [e.top() for e in elution_samples_multi],
        new_tip='never'
    )
    m50.drop_tip()
    for e in elution_samples_multi:
        pick_up(m300)
        mix(5, 80, e, m300)
        m300.drop_tip()

    robot.pause('Incubate at room temperature for 5 minutes, and then replace \
plate on magnetic module with elution plate from slot 2. Place a fresh \
elution plate in slot 2.')
    ...
    for m in mag_samples_multi:
        pick_up(m300)
        m300.aspirate(200, m.bottom(0.5))
        m300.delay(seconds=3)
        m300.dispense(200, liquid_waste[2])
        m300.drop_tip()
...
liquid waste and replace reagents in the 12-channel reservoir.')
