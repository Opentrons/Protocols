...
def run(protocol):
    ...
    mmPlates = [
        protocol.load_labware(
            ...
            s) for s in ['3', '6']]
    plate1 = mmPlates[0].wells()[:192]
    plate2 = mmPlates[0].wells()[192:]
    plate3 = mmPlates[1].wells()[:192]
    plate4 = mmPlates[1].wells()[192:]

    for s_plate, d_plate in zip(dnaPlates[:2], [plate1, plate2]):
        dest1 = d_plate[::2]
        dest2 = d_plate[1::2]
        for src, d1, d2 in zip(s_plate.wells()[:], dest1, dest2):
            pip.pick_up_tip()
            pip.aspirate(5, src)
            pip.dispense(2.5, d1)
            pip.dispense(2.5, d2)
