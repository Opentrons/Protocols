from opentrons import labware, instruments, robot, modules

metadata = {
    'protocolName': 'BioFluid Mix and Transfer - Part 1/2',
    'author': 'Chaz <protocols@opentrons.com>',
    'source': 'Custom Protocol Request'
}

# custom labware creation
cryo_container = 'custom_agilent_cryovial_container Jan 27'
if cryo_container not in labware.list():
    labware.create(
        cryo_container,
        grid=(5, 10),
        spacing=(18.87, 18.67),
        diameter=12,
        depth=48,
        volume=6000
    )

centr_container = 'custom_centrifuge_tube_container JAN27'
if centr_container not in labware.list():
    labware.create(
        centr_container,
        grid=(8, 12),
        spacing=(13.14, 16),
        diameter=11,
        depth=30.5,
        volume=500
    )

nano_container = 'custom_nanosep_tube_container Jan27'
if nano_container not in labware.list():
    labware.create(
        nano_container,
        grid=(8, 12),
        spacing=(13.14, 16),
        diameter=8,
        depth=10,
        volume=500
    )

nano_container2 = 'custom_nanosep_tube_container2 Jan27'
if nano_container2 not in labware.list():
    labware.create(
        nano_container2,
        grid=(8, 12),
        spacing=(13.14, 16),
        diameter=8,
        depth=10,
        volume=500
    )

cryovials = labware.load(nano_container2, '3')
centtubes = labware.load(centr_container, '2')
ultratubes = labware.load(nano_container, '1')
tempdeck = modules.load('tempdeck', '10')

track = 'opentrons_24_aluminumblock_nest_1.5ml_snapcap'
if track not in labware.list():
    labware.create(
        track,
        grid=(6, 4),
        spacing=(17.25, 17.25),
        diameter=10.20,
        depth=37.9,
        volume=1500
    )
temprack = labware.load(track, '10',
                        share=True)
tempdeck.set_temperature(4)
tipracks = labware.load('opentrons_96_tiprack_300ul', '11', 'Tiprack 50/300ul')


def run_custom_protocol(
        p50_mount: 'StringSelection...' = 'left',
        p300_mount: 'StringSelection...' = 'right'
):

    # create pipette

    pip50 = instruments.P50_Single(mount=p50_mount)
    pip300 = instruments.P300_Single(mount=p300_mount)

    tip_max = 96
    tip_count = 0

    def pick_up(pip):
        nonlocal tip_count

        if tip_count == tip_max:
            robot.pause(
                'Replace 50/300ul tiprack in slot 11. \
                When ready, click RESUME.')
            pip50.reset()
            tip_count = 0
        if pip == pip50:
            pip50.pick_up_tip(tipracks.wells(tip_count))
        else:
            pip300.pick_up_tip(tipracks.wells(tip_count))
        tip_count += 1

    # Step 1 - mix bio-fluid and transfer 50uL
    # change to 48 wells.
    """
    The code below creates a new list of wells that is based on 48 wells.
    The 'centtubes' and 'ultratubes' are converted into lists that span rows
    A, C, E, G, I, and K (how the rows are named in this orientation).
    """
    cryovials48 = cryovials.rows('A') + cryovials.rows('C') + cryovials.rows('E') + cryovials.rows('G') + cryovials.rows('I') + cryovials.rows('K')
    centtubes48 = centtubes.rows('A') + centtubes.rows('C') + centtubes.rows('E') + centtubes.rows('G') + centtubes.rows('I') + centtubes.rows('K')
    ultratubes48 = ultratubes.rows('A') + ultratubes.rows('C') + ultratubes.rows('E') + ultratubes.rows('G') + ultratubes.rows('I') + ultratubes.rows('K')

    """
    Additional Step - The robot should take samples from 24 wells and transfer
    to D1 of aluminum block in slot 11, then repeat with transfer to D2.
    """

    c6 = temprack.wells('C6')
    d6 = temprack.wells('D6')
    vialcounter = 0

    for source, dest in zip(cryovials48, centtubes48):
        pick_up(pip300)
        pip300.transfer(50, source, dest, new_tip='never')
        pip300.drop_tip()
        pip300.drop_tip()

    # Step 2 - transfer 40ul aliquot of solution
    tempdeck.wait_for_temp()

    for dest in centtubes48:
        pick_up(pip300)
        pip300.transfer(40, temprack.wells(0), dest,
                       new_tip='never')
        pip300.mix(3, 35, dest)
        pip300.blow_out(dest.top())
        pip300.drop_tip()
        pip300.drop_tip()

    # Step 3 + 4 transfer 110ul solution then transfer 200 to nanosep tubes

    for k, cent, ultra in zip(range(48), centtubes48, ultratubes48):
        m = (k//10) + 1
        pick_up(pip300)
        pip300.transfer(110, temprack.wells(m), cent,
                        new_tip='never')
        pip300.mix(3, 150, cent)
        pip300.blow_out(cent.top())
        pip300.transfer(200, cent, ultra,
                        new_tip='never')
        pip300.blow_out(ultra.top())
        pip300.drop_tip()
        pip300.drop_tip()

    robot.comment("Part 1 is now complete. Please remove samples from OT-2 for \
    centrifugation. After centrifugation, replace samples on the deck and run \
    Part 2. Be sure to replace the cryovials in slots 3/6/9 with sample \
    vials. Lastly, replace the tiprack in slot 11 with a full rack.")


run_custom_protocol(**{'p50_mount': 'left', 'p300_mount': 'right'})
