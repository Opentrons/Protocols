...
def run(ctx):
    ...
    for mass, mol in zip(sample_mass, sample_mol):
        sample_mol.append(mass/mol)
    for mol, vol in zip(sample_mol, sample_vol):
        start_conc.append(mol/vol)
    # final conc can be calculated, set by variable, or imported via CSV
    # depending on what client says
    final_conc = 5  # nmols/uL
    tot_vol = 30
    transfer_vol = []
    nfw_vol = []
