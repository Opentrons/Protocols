...
metadata = {
    'protocolName': 'Second Capture (Part 5/8)',
...
def run_custom_protocol(
    ...
    pools = samples_rack.wells('E2', length=number_of_pools)
    mag_pools = mag_plate.wells('E2', length=number_of_pools)
    ...
    wash_dests = [wash_rack.wells(i*2+24, length=2)
    ...
    new_samples_loc = mag_plate.wells('A3', length=number_of_pools)
    wash_dests_temp = [block.wells(i*2+24, length=2)
                       for i in range(number_of_pools)]
    ...
    new_samples_loc = samples_rack.wells('A3', length=number_of_pools)
    ...
    # transfer samples to wash rack
    wash_dests_temp = [block.wells(i*2+32, length=2)
                       for i in range(number_of_pools)]
    ...
    new_samples_loc = mag_plate.wells('E3', length=number_of_pools)
    for wash_set, s in zip(wash_dests_temp, new_samples_loc):
        m300.pick_up_tip(tips300[tip300_counter])
        m300.transfer(100, wash_set, s, new_tip='never')
        m300.drop_tip()
        tip300_counter += 1
    ...
    for s in samples_rack.wells('E3', length=number_of_pools):
        m10.pick_up_tip(tips10[tip10_counter])
    ...
    samples_mag = mag_plate.wells('E3', length=number_of_pools)
    wash_dests_temp = block.wells('A6', length=number_of_pools)
    for _ in range(2):
