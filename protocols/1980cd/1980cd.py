...
def run(ctx):
    ...
    step, sample_plates = get_values('step', 'sample_plates')
    ...
    if step == 1:
        ...
        trough = ctx.load_labware('nest_1_reservoir_195ml','2').wells()[0]
        plates = [ctx.load_labware('nest_96_wellplate_200ul_flat', str(i)) for i in range(3,12)[:sample_plates]]
        for plate in plates:
            p300m.pick_up_tip()
    ...
    if step == 2:
        ...
        reagents = ctx.load_labware('nest_12_reservoir_15ml', '2')
        control_1 = reagents.wells_by_name()["A1"]
        control_2 = reagents.wells_by_name()["A2"]
        
        sample_plate = ctx.load_labware('nest_96_wellplate_200ul_flat', '3')
